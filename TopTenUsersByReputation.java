...
public class TopTenUsersByReputation extends Configured implements Tool {
  ...
  public static class TopTenMapper extends Mapper<Object, Text, IntWritable, TextArrayWritable> {
    ...
    public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
      ...
      if (isNullOrEmpty(userId) || isNullOrEmpty(reputation) || !isInteger(reputation) || isNullOrEmpty(displayName)) {
        return;
      }
    ...
    protected void cleanup(Context context) throws IOException, InterruptedException {
      for (int key : repToRecordMap.keySet()) {
        ...
        context.write(new IntWritable(key), TextArrayWritable.from(pair));
      }
    }
  }

  /**
   * In the book example there is no cleanup because a NullWritable is used as the key, so reduce only happens once.
   * Because I'm using an IntWritable for the key I needed to output the tree during cleanup, just like the mapper.
   * @author cfeduke
   *
   */
  public static class TopTenReducer extends Reducer<IntWritable, TextArrayWritable, IntWritable, TextArrayWritable> {
    private TreeMap<Integer, Pair<Writable, Writable>> repToRecordMap = new TreeMap<Integer, Pair<Writable, Writable>>();

    @Override
    public void reduce(IntWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException,
        InterruptedException {
      for (ArrayWritable value : values) {
        repToRecordMap.put(Integer.valueOf(key.get()), new Pair<Writable, Writable>(value.get()[0], value.get()[1]));
        if (repToRecordMap.size() > 10) {
          repToRecordMap.remove(repToRecordMap.firstKey());
        }
      }
    ...
    public void cleanup(Context context) throws IOException, InterruptedException {
      for (int reputation : repToRecordMap.descendingKeySet()) {
        ...
        context.write(new IntWritable(reputation), new TextArrayWritable(pair));
      }
    }
  }
